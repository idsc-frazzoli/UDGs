...
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
   ...
   OUTPUT = MPCCcar_py.MPCCcar_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['xinit'] - column vector of length 9
       PARAMS['x0'] - column vector of length 780
       PARAMS['all_parameters'] - column vector of length 3780

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['all_var'] - column vector of size 780

   [OUTPUT, EXITFLAG] = MPCCcar_py.MPCCcar_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = MPCCcar_py.MPCCcar_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'MPCCcar/lib/MPCCcar.dll')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCCcar/lib/MPCCcar_withModel.dll'))
    csolver = getattr(_lib,'MPCCcar_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCCcar/lib/libMPCCcar_withModel.dll'))
    csolver = getattr(_lib,'MPCCcar_solve')

class MPCCcar_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 9),
('x0', ctypes.c_double * 780),
('all_parameters', ctypes.c_double * 3780),
]

MPCCcar_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
MPCCcar_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class MPCCcar_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('all_var', ctypes.c_double * 780),
]

MPCCcar_outputs = {'all_var' : np.array([]),
}


class MPCCcar_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(MPCCcar_params_ctypes), ctypes.POINTER(MPCCcar_outputs_ctypes), ctypes.POINTER(MPCCcar_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def MPCCcar_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = MPCCcar_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=MPCCcar_params_types[par], requirements='F')
    ...
    outputs_py = MPCCcar_outputs_ctypes()
    info_py = MPCCcar_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.MPCCcar_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.MPCCcar_casadi2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.MPCCcar_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.MPCCcar_casadi2forces)
            else:
                exitflag = _lib.MPCCcar_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.MPCCcar_casadi2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in MPCCcar_outputs:
        MPCCcar_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return MPCCcar_outputs,int(exitflag),info_py
