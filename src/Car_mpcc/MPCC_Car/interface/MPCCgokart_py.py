...
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
   ...
   OUTPUT = MPCCgokart_py.MPCCgokart_solve(PARAMS) solves a multistage problem
   ...
   [OUTPUT, EXITFLAG] = MPCCgokart_py.MPCCgokart_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = MPCCgokart_py.MPCCgokart_solve(PARAMS) returns 
...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'MPCCgokart/lib/MPCCgokart.dll')) 
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCCgokart/lib/MPCCgokart_withModel.dll'))
    csolver = getattr(_lib,'MPCCgokart_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'MPCCgokart/lib/libMPCCgokart_withModel.dll'))
    csolver = getattr(_lib,'MPCCgokart_solve')

class MPCCgokart_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 8),
('x0', ctypes.c_double * 372),
('all_parameters', ctypes.c_double * 2015),
]

MPCCgokart_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
MPCCgokart_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
}

class MPCCgokart_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('all_var', ctypes.c_double * 372),
]

MPCCgokart_outputs = {'all_var' : np.array([]),
}


class MPCCgokart_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
csolver.argtypes = ( ctypes.POINTER(MPCCgokart_params_ctypes), ctypes.POINTER(MPCCgokart_outputs_ctypes), ctypes.POINTER(MPCCgokart_info), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

def MPCCgokart_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCES PRO v4.1.1
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = MPCCgokart_params_ctypes()
    for par in params_arg:
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=MPCCgokart_params_types[par], requirements='F')
    ...
    outputs_py = MPCCgokart_outputs_ctypes()
    info_py = MPCCgokart_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.MPCCgokart_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp) , _lib.MPCCgokart_casadi2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.MPCCgokart_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None , _lib.MPCCgokart_casadi2forces)
            else:
                exitflag = _lib.MPCCgokart_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp , _lib.MPCCgokart_casadi2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in MPCCgokart_outputs:
        MPCCgokart_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return MPCCgokart_outputs,int(exitflag),info_py

solve = MPCCgokart_solve
